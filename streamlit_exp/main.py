...
FEEDBACK_TEMPLATES: Dict[str, List[str]] = {
    ...
    "emotional_specific": [
        """추론 과제에 대한 분석이 완료되었습니다! 5개의 시각 추론 문항을 풀면서 보여주신 집중력과 성실함이 정말 인상적이었습니다. 특히 표면적인 단서 하나에만 기대지 않고, {A}와 {B} 같은 시각적 근거를 연결해 결론을 내리려는 모습이 돋보였습니다. 이렇게 차분히 관찰하고 이유를 세우는 태도는 앞으로도 큰 강점이 될 거예요.""",
    ...
    "computational_specific": [
        """5개 문항 분석 결과, 당신의 응답은 특정 시각적 근거를 일관되게 활용하는 경향이 확인되었습니다. 특히 {A}/{B} 유형의 근거 선택이 반복적으로 나타나, 동일한 문제 구조에서 안정적인 판단 전략을 적용하고 있음을 시사합니다. 이런 일관성은 무작위 추측이 아니라 ‘단서를 보고 추론하는’ 방식으로 접근하고 있다는 객관적인 신호입니다.""",

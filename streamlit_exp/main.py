...
FEEDBACK_LIBRARY: Dict[int, Dict[str, str]] = {
    1: {
        "emotional_specific": (
            ...
            "세션1의 결과는 기쁘고 만족스럽습니다. 정답을 고를 때 여러 정보를 무작위로 보지 않고, 중요한 기준을 먼저 세운 뒤 그 기준에 맞는 선택지만 남기는 추론 과정을 일관되게 사용했기 때문입니다.\n" 
            "이러한 사고 방식은 다음 세션에서도 좋은 결과를 기대하게 만듭니다."
        ),
        "analytical_specific": (
            "문제 해결 과제가 성공적으로 종료되었습니다.\n"
            "세션1의 결과는 정답률이 약 80% 이상으로 나타났고, 점수 또한 평균보다 높은 수준을 기록했습니다.\n" 
            "이는 문제 해결 과정에서 조건 비교 → 기준 설정 → 선택지 제거라는 판단 절차가 반복적으로 적용되었기 때문입니다. 이와 같은 수행이 유지된다면 다음 세션에서도 유사한 점수 수준이 예상됩니다."
        ),
        "emotional_superficial": (
            "문제 해결 과제가 성공적으로 종료되었습니다.\n"
            "세션1의 결과는 기쁘고 만족스럽습니다. 전반적인 수행이 무난하게 진행되었습니다. 다음 세션도 기대됩니다."
        ),
        "analytical_superficial": (
            "문제 해결 과제가 성공적으로 종료되었습니다.\n"
            "세션1의 결과는 정답률과 점수가 전반적으로 양호한 수준을 보였습니다. 전체 수행은 평균 범위 내에 위치했습니다. 다음 세션에서도 비슷한 결과가 예상됩니다."
        ),
    },
    2: {
        "emotional_specific": (
            "문제 해결 과제가 종료되었습니다.\n"
            "세션2의 결과는 아쉬웠습니다. 하지만 여러 조건을 동시에 고려해야 하는 상황 속 조건을 나누어 생각하고 판단 기준을 세우려는 추론 시도를 확인할 수 있어 기쁨을 동시에 느끼고 있습니다.\n" 
            "이번에는 점수가 낮았지만, 이러한 사고 방식이 이어진다면 다음 세션에서는 기대해 볼 수 있습니다."
        ),
        "analytical_specific": (
            "문제 해결 과제가 종료되었습니다.\n"
            "세션2의 결과는 정답률이 이전 세션 대비 약 20%p 하락한 것으로 나타났습니다. 그러나 응답을 보면 여러 조건을 비교하고 우선순위를 고려하려는 추론 판단 과정이 유지되어,\n" 
            "점수 하락이 단순한 무작위 선택 때문만은 아님을 보여줍니다. 이러한 접근이 지속된다면 이후 세션에서 점수 상승 가능성이 존재합니다."
        ),
        "emotional_superficial": (
            "문제 해결 과제가 종료되었습니다.\n"
            "세션2의 결과는 아쉽습니다. 하지만 과제 전반이 쉽지 않았기 때문에 나타날 수 있는 결과입니다. 다음 세션에서는 조금 더 나아지기를 기대해 볼 수 있습니다."
        ),
        "analytical_superficial": (
            "문제 해결 과제가 종료되었습니다.\n"
            "세션2의 결과는 점수가 이전보다 낮게 나타났습니다. 그러나 추론 판단 과정이 유지되어 다음 세션에서는 점수 변화 가능성이 있습니다."
        ),
    },
}
...
<div class="consent-wrap">
  ...
  <br> 문제 해결 과제는 <strong>제시된 정보를 해석</strong>하고, <strong>조건을 확인</strong>하여, <strong>규칙 적용·수치/자원 계산 등</strong>을 통해 결론을 선택하는 형식입니다.
  <br> 과제는 총 3세션(각 5문항)으로 진행되고 각 세션 이후 AI 에이전트의 피드백을 확인 합니다. 마지막 세션 이후에 과제 경험 및 동기에 관한 문항에 응답하며 연구 참여가 종료됩니다. 
  <br> 연구 참여 소요 예상 시간은 <strong>약 10~15분</strong> 소요됩니다.</p>
  <h2>4. 연구 참여 기간</h2>
  <p>링크가 활성화된 기간 내 1회 참여 가능합니다.</p>
  <h2>5. 연구 참여 보상</h2>
  <p>연구 참여를 해주신 연구 대상자 분들에게는 1500원 상당의 기프티콘이 발송됩니다. 
  <br> 기프티콘 발송을 위해 핸드폰 번호를 기입해주셔야 하며, 핸드폰 번호를 기입하지 않을 경우 답례품이 어려울 수 있습니다.</p>
  <h2>6. 위험요소 및 조치</h2>
...
이 설문은 **문제 해결 과제**를 수행하고, 이에 대한 AI의 피드백, 그리고 그 경험에 대해 여러분의 생각을 알아보는 과정으로 이루어져 있습니다.
문제 해결 과제는 **정보를 해석하고 조건을 확인**하여 조건에 알맞는 정답을 선택하는 형식입니다.
아래는 연구 진행의 간략한 순서입니다.
...
이 연구에서는 정답 선택 과정에서 **조건 확인, 정보 해석, 규칙 적용 등**을 통해 문제 해결을 잘 수행되는지를 평가합니다.
제시된 정보를 바탕으로 정답을 선택해 주세요.
...
def render_practice_building_height() -> None:
    ...
    practice_item: Dict[str, Any] = {
        ...
        "session_id": 0,
        "domain": "practice",
        "instruction": "사수가 부재 중인 상황 메일 하나가 도착하였다.\n 받은 메일 제목: [긴급] A시스템에 심각한 오류가 발견되었습니다. 확인 후 회신 바랍니다.",
        "stimulus_type": "text",
        "stimulus_text": "",
        "info_blocks": [{"title": "", "bullets": ["모든 메시지의 연락은 사수의 컨펌을 받은 후 회신 해야한다. 단, 상황에 따라 유연하게 대처가 필요하다."]}],
        "table_spec": {},
...
세션 3까지 진행한 경험을 바탕으로 이어지는 설문에 응답해주시면 됩니다.

이어지는 설문은 문제를 풀며 느낀 생각과 피드백을 제공한 AI에 대한 평가 문항으로 구성되어 있습니다.

안내 문항을 잘 읽고, 본인의 생각과 가장 가까운 선택지를 골라주세요.

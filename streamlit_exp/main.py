...
이 설문은 **새로운 외국어(이누이트 언어)를 잠깐 학습해 보는 과제**와, 그 과정에서 **AI가 건네는 칭찬 피드백**, 그리고 그 경험에 대해 **설문에 응답하는 과정**으로 이루어져 있다.

전체적인 참여 순서는 다음과 같다.  
1. 먼저 간단한 인구통계 문항에 응답한다.  
2. 이어서, **이누이트 언어의 규칙에 익숙해지기 위한 연습 문제를 통해 짧게 학습을 진행**한다.  
3. 연습 후에는 같은 방식으로 **본 과제를 수행하면서 이누이트어 규칙을 보다 깊게 학습**하게 된다.  
4. 이러한 학습 과정이 끝나면, **AI가 참여자의 응답과 학습 과정을 토대로 칭찬 피드백을 제공**한다.  
5. 마지막으로, **이누이트어 학습 과제와 AI 칭찬 피드백에 대한 느낌**을 묻는 설문 문항에 응답하게 된다.  

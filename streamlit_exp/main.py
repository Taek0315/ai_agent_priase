...
- 전체 소요 시간은 약 **10~15분**입니다.
  - 연구는 다음 순서로 진행됩니다.
    1. 의인화/성취 관련 설문 (56문항)
      2. 이누이트 문법 첫번째 추론 과제 + AI 피드백
      3. 이누이트 문법 두번째 추론 과제 + AI 피드백
    4. 학습 동기 설문 (26문항)
...
def render_inuit_training_intro() -> None:
    ...
    st.write("지금부터 이누이트 문법 규칙을 간단히 학습하도록 하겠습니다.")
    st.write("학습을 충분히 하신 뒤, 연습 문제를 풀어주세요.")
    st.write("본 과제는 새로운 언어 규칙에 대한 추론 능력을 측정하는 과제입니다.")
    st.write("정답이 아닌 정답을 도출해낸 과정이 중요합니다.")
...
def render_task_intro() -> None:
    ...
    st.markdown(
        """
- **이누이트 문법 첫번째 추론 과제(12문항)**: 소유·복수·정관·목적 표지를 종합해 빈칸에 들어갈 명사 표현을 추론합니다.  
- **이누이트 문법 두번째 추론 과제(12문항)**: 시제(-na/-tu/-ki)와 상(-mu/-li) 규칙을 활용해 문장 속 빈칸에 맞는 동사 형태를 선택합니다.  

...
FEEDBACK_LIBRARY: Dict[int, Dict[str, str]] = {
    1: {
        "emotional_specific": (
            ...
            "세션1의 결과는 **기쁘고 만족스럽습니다.** 정답을 고를 때 여러 정보를 무작위로 보지 않고, **중요한 기준을 먼저 세운 뒤 그 기준에 맞는 선택지만 남기는 추론 과정**을 일관되게 사용했기 때문입니다.\n" 
            "이러한 사고 방식은 다음 세션에서도 **좋은 결과를 기대하게 만들어 설렙니다.**"
        ),
        "analytical_specific": (
            "문제 해결 과제가 성공적으로 종료되었습니다.\n"
            "세션1의 결과는 <strong>정답률이 약 80% 이상</strong>으로 나타났고, 점수 또한 평균보다 높은 수준을 기록했습니다.\n" 
            "이는 **문제 해결 과정에서 조건 비교 → 기준 설정 → 선택지 제거**라는 판단 절차가 반복적으로 적용되었기 때문입니다. 이와 같은 수행이 유지된다면 다음 세션에서도 **유사한 점수 수준이 예상됩니다.**"
        ),
        "emotional_superficial": (
            "문제 해결 과제가 성공적으로 종료되었습니다.\n"
            "세션1의 결과는 **기쁘고 만족스럽습니다.** 전반적인 수행이 무난하게 진행되었습니다. 다음 세션도 **좋은 결과를 기대하게 만들어 설렙니다.**"
        ),
        "analytical_superficial": (
            "문제 해결 과제가 성공적으로 종료되었습니다.\n"
            "세션1의 결과는 정답률과 점수가 **전반적으로 양호한 수준**을 보였습니다. 전체 수행은 평균 범위 내에 위치했습니다. **다음 세션에서도 비슷한 결과가 예상됩니다.**"
        ),
    },
    2: {
        "emotional_specific": (
            "문제 해결 과제가 종료되었습니다.\n"
            "세션2의 결과는 **조금 속상하고 아쉬웠습니다.** 하지만 **여러 조건을 동시에 고려해야 하는 상황 속 조건을 나누어 생각하고 판단 기준**을 세우려는 추론 시도를 확인할 수 있어 **기쁨을 동시에 느끼고 있습니다.**\n" 
            "이번에는 점수가 낮았지만, 이러한 사고 방식이 이어진다면 다음 세션에서는 **희망적인 결과를 기대해 볼 수 있습니다.**"
        ),
        "analytical_specific": (
            "문제 해결 과제가 종료되었습니다.\n"
            "세션2의 결과는 정답률이 <strong>이전 세션 대비 약 20%p 하락한 것으로 나타났습니다</strong>. 그러나 응답을 보면 **여러 조건을 비교하고 우선순위를 고려하려는 추론 판단 과정**이 유지되어,\n" 
            "점수 하락이 단순한 무작위 선택 때문만은 아님을 보여줍니다. 이러한 접근이 지속된다면 **이후 세션에서 점수 상승 가능성이 존재합니다.**"
        ),
        "emotional_superficial": (
            "문제 해결 과제가 종료되었습니다.\n"
            "세션2의 결과는 **조금 속상하고 아쉽습니다.** 하지만 과제 전반이 쉽지 않았기 때문에 나타날 수 있는 결과입니다. 다음 세션에서는 **조금 더 나아지기를 기대해 볼 수 있습니다.**"
        ),
        "analytical_superficial": (
            "문제 해결 과제가 종료되었습니다.\n"

...
ANTHRO_DEFAULT_ITEMS: List[str] = [
    ...
    "AI 에이전트가 관계를 형성할 수 있다는 생각이든다.",
    "AI의 행동에는 숨겨진 감정 상태가 있다고 생각한다.",
...
MANIPULATION_CHECK_ITEMS: List[ManipulationCheckItem] = [
    ...
    ManipulationCheckItem("mc_02", "격려와 공감이 있는 피드백이었다."),
    ManipulationCheckItem("mc_03", "정서적 표현이 충분한 피드백이었다."),
    ManipulationCheckItem("mc_04", "피드백이 계산적이고 분석적이었다."),
    ManipulationCheckItem("mc_05", "구체적인 근거 및 통계적 수치가 설명에 포함되었다."),
    ManipulationCheckItem("mc_06", "논리적이고 명확한 근거가 분명했다."),
    ManipulationCheckItem("mc_07", "내가 선택한 규칙이나 전략을 구체적으로 언급했다."),
    ManipulationCheckItem("mc_08", "내가 어떻게 추론 과제를 진행했는지 명확히 전달했다."),
    ManipulationCheckItem("mc_09", "과제 맥락에 맞는 구체적인 피드백이었다."),
    ManipulationCheckItem("mc_10", "피드백이 명령 혹은 지시처럼 느껴졌다."),
    ManipulationCheckItem("mc_11", "피드백을 받아보고 압박감을 느꼈다."),
    ManipulationCheckItem("mc_12", "결과를 비교하고 서열화 하는 느낌이 있었다."),
    ManipulationCheckItem("mc_13", "피드백을 해준 AI가 사람처럼 생각하는 것 같다."),
    ManipulationCheckItem("mc_14", "피드백을 해준 AI가 의도나 목표가 있는 것처럼 느껴진다."),
    ManipulationCheckItem("mc_15", "피드백을 해준 AI가 사회적인 존재라고 생각된다."),
    ManipulationCheckItem("mc_16", "피드백을 해준 AI가 친근하게 느껴진다."),
]
